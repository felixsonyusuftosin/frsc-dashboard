...
//usermodel
signup(usernamel, passwordl, firstnamel, lastnamel, groupl):Promise<any>{
//username user's username
// password : users password
//group: level {public, admin, normalresponse}
let th = this;
return new Promise(resolve =>{
th.postdata({ _id: usernamel + passwordl, username: usernamel, firstname:firstnamel, lastname:lastnamel, password: passwordl, group:groupl}, this.usermodel)
.then((dat) =>{
    resolve(dat);
})
.catch((err) => {
...
updatepicture(img, dbname, username, password):Promise<any>{
  return new Promise(resolve =>{
    ...
    .then((dat)=>{
      ...
      {
  let remote = this.remote + '/' + dbname + '/' + dat._id + '/' + img + '?rev=' + dat._rev;
  let  headers = this.headers; 
  this.http.put(remote, {headers: headers})
     .map(res => res.json())
      .subscribe(
       (data) => {
